...
iCache 
    :: forall tagBits indexBits lineBits ways ways'. ((tagBits + (indexBits + lineBits)) ~ 30, ways ~ (ways' + 1), KnownNat indexBits, KnownNat tagBits, KnownNat lineBits, KnownNat ways)
    => SNat tagBits
    -> SNat indexBits
    -> SNat ways
...
iCache _ _ _ replacementFunc req reqAddress fromMemValid fromMemData = (respValid, respData, busReq, busReqAddress)
    ...
    writes :: Vec ways (Signal (CacheWrite indexBits tagBits lineBits))
